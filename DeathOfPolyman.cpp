...
void drawground(float[], float[], int, float, float, float, int);//draw polyground
void drawlines(float[], float[], int, int, float);
...
void RenderScene(void) {
	...
	float bopx[6], bopy[6];//mouth open
	...
	settransgrnd();
	drawground(gpx, gpy, 5, 0.1, 0.4, 0.0, 2);
	if (storm) {
	...
	if (storm) {
		...
		for (int i = 0; i < 11; i++) {
			settransrain();
			drawlines(rx1, ry1, 22, i, 1);
			drawlines(rx2, ry2, 22, i, 1);
		}
	}
	if (striking) {
		glColor3f(1.0, 1.0, 1.0);
		...
		glLineWidth(3);
		drawlines(lgtx, lgty, 10, 0, 1.5);
	}
	glLineWidth(1);
	for (int i = 0; i < 5; i++) {
		settranscldlft(i);
		drawground(cpx, cpy, 7, .2, .2, .2, 0);
		settranscldrgt(i);
		drawground(cpx, cpy, 7, .2, .2, .2, 0);
	}
	...
	if (!closedw) {
		drawpm(bopwx, bopwy, 5, 0.0, 0.5, 0.0, 0.5);
	}
	glColor3f(0.5, 0.0, 0.5);
	drawlines(pwcrx, pwcry, 8, 0, .51);
	drawlines(pwcrx, pwcry, 8, 0, -.51);
	glPointSize(5);
	draweye(epwx, epwy);
	glColor3f(0.0, 0.0, 0.0);
	settranspwff();
	drawfoot(ffpwx, ffpwy, -.51);

	//squareman render
	glColor3f(0.0, 0.0, 0.0);
	settranssqrf();
	drawfoot(rfsx, rfsy, .51);
	...
	glColor3f(0.0, 0.0, 0.0);
	settranssqff();
	drawfoot(ffsx, ffsy, -.51);
	settransswd();
	drawswd(swsx, swsy, 0.0, 0.0, 0.0);

	//polyman render
	settransrf();
	drawfoot(rfpx, rfpy, .51);//polyman rearfoot
	...
	if (!closed) {
		drawpm(bopx, bopy, 5, 0.5, 0.0, 0.5, 0.5);//polyman mouth open
	}
	...
	if (dead) {
		drawlines(depx, depy, 4, 0, -1);
		glColor3f(1.0, 0.0, 0.0);
		glLineWidth(3);
		drawlines(bldx, bldy, 12, 0, -1);
		glLineWidth(1);
	}
	glColor3f(0.0, 0.0, 0.0);
	settransff();
	drawfoot(ffpx, ffpy, -.51);//polyman front foot
...
void loadbackground(float gpx[], float gpy[], float cpx[], float cpy[], float skyx[], float skyy[], float rx1[], float ry1[], float rx2[], float ry2[], float lgtx[], float lgty[]) {
	...
	skyx[0] = 11.0;  skyy[0] = 11.0;
	skyx[1] = -11.0; skyy[1] = 11.0;
	skyx[2] = -11.0; skyy[2] = -5.0;
	skyx[3] = 11.0;  skyy[3] = -5.0;
	skyx[4] = 11.0;  skyy[4] = 11.0;
...
void loadpm(float px[], float py[], float bpx[], float bpy[], float bopx[], float bopy[], float ffpx[], float ffpy[], float rfpx[], float rfpy[], float epx[], float epy[], float depx[], float depy[], float bldx[], float bldy[]) {
	...
	bopx[0] = -.375;  bopy[0] = 0.0;
	bopx[1] = -.875;  bopy[1] = -.5;
	bopx[2] = -.625;  bopy[2] = -.75;
	bopx[3] = .625;   bopy[3] = -.75;
	bopx[4] = 1.125;  bopy[4] = 0.0;
	bopx[5] = -.375;  bopy[5] = 0.0;
	...
	bldx[0] = -.675 + .15;  bldy[0] = .75;
	bldx[1] = -.675 + .15;  bldy[1] = .5;
	bldx[2] = -1.125 + .15;    bldy[2] = 0;
	bldx[3] = -1.125 + .15;    bldy[3] = -.25;
	bldx[4] = 1.125 + .15;     bldy[4] = 0;
	bldx[5] = 1.125 + .15;     bldy[5] = -.25;
	bldx[6] = -.675 + .15;  bldy[6] = -.75;
	bldx[7] = -.675 + .15;  bldy[7] = -1;
	bldx[8] = .675 + .15;   bldy[8] = -.75;
	bldx[9] = .675 + .15;   bldy[9] = -1;
	bldx[10] = .675 + .15; bldy[10] = .75;
	bldx[11] = .675 + .15; bldy[11] = .5;
	return;
...
void loadpw(float pwx[], float pwy[], float bpwx[], float bpwy[], float bopwx[], float bopwy[], float ffpwx[], float ffpwy[], float rfpwx[], float rfpwy[], float epwx[], float epwy[],
	...
	bopwx[0] = -.375;  bopwy[0] = 0.0;
	bopwx[1] = -.875;  bopwy[1] = -.5;
	bopwx[2] = -.625;  bopwy[2] = -.75;
	bopwx[3] = .625;   bopwy[3] = -.75;
	bopwx[4] = 1.125;  bopwy[4] = 0.0;
	bopwx[5] = -.375;  bopwy[5] = 0.0;
...
void drawlines(float pwcrx[], float pwcry[], int length, int j, float z) {
	cout << "in drawlines" << endl;
	glBegin(GL_LINES);
	int k = 0;
	if (j > 0)k = j;
	for (int i = 0; i < length; i++) {
		glVertex3f(pwcrx[i], pwcry[i] + j + k, z);
	}
	glEnd();
	glFlush();
	return;
...
void drawpm(float pxp[], float pyp[], int j, float r, float g, float b, float z) {
	...
	for (i = 0; i < k - 1; i++) {
		...
		glVertex3f(pxp[i], pyp[i], z - 1);
		glVertex3f(pxp[i], pyp[i], z);
		if (i < k - 1)glVertex3f(pxp[i + 1], pyp[i + 1], z);
		else glVertex3f(pxp[0], pyp[0], z);
		if (i < k - 1)glVertex3f(pxp[i + 1], pyp[i + 1], z - 1);
	...
	for (i = 0; i < j - 1; i++) {
		...
		glVertex3f(pxp[i], pyp[i], z - 1);
		glVertex3f(pxp[i], pyp[i], z);
		glVertex3f(pxp[i + 1], pyp[i + 1], z);
		glVertex3f(pxp[i + 1], pyp[i + 1], z - 1);
		glVertex3f(pxp[i], pyp[i], z - 1);
		glEnd();
	}
	for (i = 0; i < j - 1; i++) {
...
void drawswd(float pxp[], float pyp[], float r, float g, float b) {
	...
	glBegin(GL_LINES);
	glVertex3f(pxp[0], pyp[0], -1);
	glVertex3f(pxp[1], pyp[1], -1);
	glEnd();
	...
	for (i = 1; i <= sizeof(pxp); i++) {
		glVertex3f(pxp[i], pyp[i], -.9);
	}
	for (i = sizeof(pxp); i >= 1; i--) {
		glVertex3f(pxp[i], pyp[i], -1.1);
	}
...
void drawground(float pxp[], float pyp[], int length, float r, float g, float b, int z) {
	int i;
...
void settranspw(void) {
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(pwx, pwy, 1.0);
	glRotatef(thetaw, 0.0, 0.0, 1.0);
	glRotatef(turnpw, 0.0, 1.0, 0.0);
	return;
}//settranspw
